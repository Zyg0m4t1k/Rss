...
class rss extends eqLogic {
	...
	public function RSSJeedom($fluxjeedom){
		...
		if (!is_object($Jeedom_Blog)) {
			...
			$Jeedom_Blog->setConfiguration("lien_rss","https://www.jeedom.com/blog//?feed=rss2");
		...
		if (!is_object($Jeedom_Market)) {
			...
			$Jeedom_Market->setConfiguration("lien_rss","https://www.jeedom.com/market/plugin.xml");
		...
		if (!is_object($Jeedom_Market_2)) {
			...
			$Jeedom_Market_2->setConfiguration("lien_rss","https://www.jeedom.com/market/widget.xml");
		...
		if (!is_object($Jeedom_Market_3)) {
			...
			$Jeedom_Market_3->setConfiguration("lien_rss","https://www.jeedom.com/market/widget.xml");
	...
	public function toHtml($_version = 'dashboard') 
	{
		...
		if(file_exists($lienfor)){
			...
			foreach ($fluxRSS as $key => $value){
				...
				$mini_widht = "500px";
					...
					$li .= '</marquee></button>';
					$li .= '<button type="button" style="float:right;width:5%;" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-expanded="false">';
		...
		$replace['#li#'] = $li;	
		return $this->postToHtml($_version, template_replace($replace, getTemplate('core', $version, 'eqLogic', 'rss')));
    }
